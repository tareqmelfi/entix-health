...
async function authenticateLocalUser(email: string, password: string) {
  ...
  if (!verifyLocalPassword(password, user.metadata)) return null;
  return user;
}
...
app.post("/api/login", async (request, reply) => {
  const body = z.object({ email: z.string().email().optional(), accessCode: z.string().min(1) }).parse(request.body);
  if (body.email) {
    const user = await authenticateLocalUser(body.email, body.accessCode);
    if (!user) return reply.code(401).send({ error: "invalid_local_login" });
    await touchUserLogin(user.email);
    setSession(reply, { slug: user.person_slug || "abo-talal", email: user.email, role: user.role, status: "active" });
    return { ok: true, person_slug: user.person_slug, email: user.email, role: user.role };
  }
...
app.get("/api/auth/google/callback", async (request, reply) => {
  ...
  if (user.status !== "active") {
    return reply.redirect(publicRedirect("?auth=pending", "access"));
  }

...
const VALID_VIEWS = new Set(["chat", "dashboard", "settings", "admin"]);
...
async function enforceReloadLock() {
  ...
  showLogin("تم قفل الجلسة بعد تحديث الصفحة. سجل الدخول من جديد لحماية الملف الصحي.");
  return true;
}
...
function documentKindLabel(document = {}) {
  ...
  if (document.document_type === "image_report") return "صورة";
  if (document.document_type === "text_report") return "نص";
  return "ملف";
}
...
function uploadVisualStatus(result, state = "ready") {
  if (state === "uploading") return "جاري الحفظ";
  if (!result) return "جاهز للرفع";
...
function trendLabel(trend) {
  if (trend.delta === null || trend.delta === undefined) return "قراءة واحدة";
  if (trend.delta > 0) return `زاد ${trend.delta}${trend.unit ? ` ${trend.unit}` : ""}`;
  if (trend.delta < 0) return `نقص ${Math.abs(trend.delta)}${trend.unit ? ` ${trend.unit}` : ""}`;
  return "ثابت";
}
...
function renderLabTrends(trends = []) {
  ...
  if (!trends.length) {
    labTrendList.innerHTML = `<div class="empty-row">الملفات محفوظة، لكن ما فيه مؤشرات رقمية مستخرجة بعد. OCR أو الاستخراج البنيوي يحتاج يكمل.</div>`;
    return;
  }
  for (const trend of trends.slice(0, 12)) {
    ...
    row.querySelector(".trend-meta").textContent = `${trendLabel(trend)} · ${trend.samples || 1} قراءة${trend.previous_value ? ` · السابق ${markerValue(trend.previous_value, trend.unit)}` : ""}`;
...
function renderMedicationList(medications = [], supplements = [], conditions = []) {
  ...
  const rows = [
    ...medications.map((item) => ({ ...item, group: "دواء" })),
    ...supplements.map((item) => ({ ...item, group: "مكمل" })),
    ...conditions.map((item) => ({ ...item, group: "حالة" }))
  ];
  if (!rows.length) {
    medicationList.innerHTML = `<div class="empty-row">ما فيه أدوية أو جرعات محفوظة حالياً. أضفها في الشات بصيغة واضحة مثل: "سجل دواء Levothyroxine 50mcg صباحاً".</div>`;
    return;
  }
  for (const item of rows.slice(0, 14)) {
    ...
    row.querySelector(".med-title").textContent = item.name || item.condition_name || "عنصر صحي";
    row.querySelector(".med-meta").textContent = [item.dose, item.frequency, item.timing, item.status, item.safety_status].filter(Boolean).join(" · ") || item.notes || item.reason || "محفوظ في الملف الصحي";
...
function clearSensitiveUi() {
  ...
  selectedFiles = [];
  selectedFileStatuses = new Map();
  for (const node of [pinnedConversations, conversationList, messages, documentsEl, settingsDocumentsEl, reportList, selectedFilesList, usersList]) {
...
function showLogin(message = "سجل الدخول للمتابعة") {
  clearSensitiveUi();
...
function addAttachmentMessage(files, state = "uploading") {
  ...
  const time = document.createElement("div");
  time.className = "message-time";
  time.textContent = state === "uploading" ? "جاري الرفع" : "ملف مرفق";
  item.append(content, time);
  messages.append(item);
...
function renderAttachmentCards(item, files, results = [], state = "ready") {
  ...
  files.forEach((file, index) => {
    ...
    icon.textContent = kind === "صورة" ? "IMG" : kind;
    ...
    if (savedDocument?.id) {
      ...
      memoryNote.textContent = "محفوظ في الذاكرة";
  ...
  time.textContent = results.length ? "تم الحفظ" : state === "uploading" ? "جاري الرفع" : "ملف مرفق";
  messages.scrollTop = messages.scrollHeight;
}
...
function openDocumentPreview(savedDocument) {
  ...
  previewTitle.textContent = savedDocument.title || "ملف محفوظ";
...
async function loadDashboard() {
  ...
  if (profileSummary) {
    profileSummary.textContent = `ملفات ${data.stats?.documents ?? 0} · تقارير ${data.stats?.reports ?? 0} · مؤشرات ${data.stats?.lab_markers ?? 0} · أدوية ${data.stats?.medications ?? 0}`;
  }
  ...
  if (!data.reports?.length) {
    ...
    for (const report of data.reports) {
      ...
      const metrics = Object.entries(report.metrics || {})
        .slice(0, 3)
        .map(([key, value]) => `${key}: ${typeof value === "object" && value ? markerValue(value.value ?? value.value_text, value.unit) : value}`)
        .join(" · ");
      row.innerHTML = `
  ...
  renderLabTrends(data.lab_trends || []);
...
async function uploadCurrentFile(filesOverride = null, options = {}) {
  ...
  const uploadProgress = showChatAttachment ? addMessage("system", `جاري حفظ ${files.length} ملف...`, "") : null;
...
function setView(view, options = {}) {
  ...
  viewTitle.textContent = nextView === "dashboard" ? "الداشبورد" : nextView === "settings" ? "الإعدادات" : nextView === "admin" ? "الأدمن" : "المحادثة";
  pinCurrentButton.classList.toggle("hidden", nextView !== "chat");
  if (nextView === "dashboard" || nextView === "settings") refreshDashboardQuietly();
}

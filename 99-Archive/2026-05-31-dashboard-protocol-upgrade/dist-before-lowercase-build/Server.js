...
import { clearGoogleState, clearSession, readGoogleState, requireAdmin, requireHmdbSecret, requireSession, requestIp, setGoogleState, setSession, timingSafeStringEqual } from "./security.js";
...
function chatTitleFromMessage(message) {
    ...
    if (!compact)
        return "محادثة صحية جديدة";
    return compact.length > 58 ? `${compact.slice(0, 58)}...` : compact;
}
function reportTitleFromText(text) {
    const lower = text.toLowerCase();
    if (lower.includes("ferritin") || text.includes("مخزون"))
        return "تقرير مخزون الحديد";
    if (lower.includes("vitamin d") || text.includes("فيتامين د"))
        return "تقرير فيتامين د";
    if (lower.includes("thyroid") || lower.includes("tsh") || text.includes("الغدة"))
        return "تقرير الغدة";
...
function reportSummary(text) {
    ...
    if (!compact)
        return "Report captured for structured review.";
...
function detectFlag(windowText) {
    const lower = windowText.toLowerCase();
    if (/(critical|panic|حرج|خطير)/i.test(windowText))
        return "critical";
    if (/(high|مرتفع|elevated|above|h\b)/i.test(windowText))
        return "high";
    if (/(low|منخفض|below|l\b)/i.test(windowText))
        return "low";
    if (/(normal|طبيعي|within range|داخل النطاق)/i.test(windowText))
        return "normal";
    if (/(out of range|خارج النطاق)/i.test(windowText))
        return "out_of_range";
    if (lower.includes("prediabetes") || lower.includes("pre diabetes"))
        return "out_of_range";
    return undefined;
}
function extractReferenceRange(windowText) {
...
function guessLabName(text) {
    if (/one\s*touch/i.test(text) || text.includes("مختبرات لمسة واحدة"))
        return "One Touch Medical Laboratories";
    if (/sulaiman|habib/i.test(text))
        return "Sulaiman Al Habib";
    if (/lab/i.test(text) || text.includes("مختبر"))
        return "Health laboratory";
    return null;
}
function markerWindow(normalized, aliases) {
...
function extractMarkerValue(windowText) {
    const latest = windowText.match(new RegExp(`(?:الجديد|new|result|reported|النتيجة|value)[^0-9<≤≥-]{0,40}([<≤≥>]?[\\s-]*\\d+(?:\\.\\d+)?)\\s*(${LAB_UNIT_RE})?`, "i"));
    if (latest?.[1])
        return { text: `${latest[1].replace(/\s+/g, "")}${latest[2] ? ` ${latest[2]}` : ""}`.trim(), numeric: normalizeNumber(latest[1]), unit: latest[2] };
...
function documentTypeForUpload(ext, mimetype) {
    if (ext === ".pdf")
        return "lab_pdf";
    if (TEXT_UPLOAD_EXTENSIONS.has(ext))
        return "text_report";
    if (mimetype?.startsWith("image/") || [".png", ".jpg", ".jpeg", ".webp", ".heic", ".heif", ".tif", ".tiff"].includes(ext))
        return "image_report";
    return "uploaded_file";
}
function previewMimeForPath(filePath, documentType) {
    const ext = path.extname(filePath || "").toLowerCase();
    if (ext === ".pdf" || documentType === "lab_pdf")
    ...
    if ([".heic", ".heif"].includes(ext))
        return "image/heic";
    if ([".txt", ".md", ".csv", ".tsv", ".json", ".xml", ".yaml", ".yml"].includes(ext) || documentType === "text_report")
        return "text/plain; charset=utf-8";
    return "application/octet-stream";
}
function inlineFilename(title) {
    return (title || "health-file").replace(/[\r\n\"]/g, " ").slice(0, 160);
}
function shouldRunOcr(ext, mimetype) {
...
async function runLocalOcr(filePath, ext, documentId) {
    if (ext === ".pdf") {
...
async function upsertLabPanelFromText(client, person, sourceDocumentId, title, text, createdBy) {
    ...
    let panel;
    if (sourceDocumentId) {
        const existing = await client.query(`select id, panel_date, lab_name, summary, created_at
       from lab_panels
       where person_id = $1 and source_document_id = $2
       order by created_at desc
       limit 1`, [person.id, sourceDocumentId]);
        if (existing.rows[0]) {
            const updated = await client.query(`update lab_panels
         set panel_date = coalesce($3::date, panel_date),
             lab_name = coalesce($4, lab_name),
             summary = $5
         where id = $1 and person_id = $2
         returning id, panel_date, lab_name, summary, created_at`, [existing.rows[0].id, person.id, panelDate || null, labName, summary]);
            panel = updated.rows[0];
            await client.query("delete from lab_markers where lab_panel_id = $1 and person_id = $2", [panel.id, person.id]);
        }
    }
    if (!panel) {
        const inserted = await client.query(`insert into lab_panels (person_id, panel_date, lab_name, source_document_id, summary, created_by)
       values ($1, $2, $3, $4, $5, $6)
       returning id, panel_date, lab_name, summary, created_at`, [person.id, panelDate || null, labName, sourceDocumentId || null, summary, createdBy || "local-parser"]);
        panel = inserted.rows[0];
    }
    for (const marker of markers) {
        await client.query(`insert into lab_markers (lab_panel_id, person_id, marker_name, value_text, value_numeric, unit, reference_range, flag, interpretation, created_by)
...
async function backfillStructuredLabMarkers(client, person) {
    const candidates = await client.query(`select d.id, d.title, d.extracted_text, d.created_by, r.id as report_snapshot_id
     from documents d
    ...
    for (const row of candidates.rows) {
        ...
        if (row.report_snapshot_id) {
            await client.query(`update report_snapshots
...
async function markDocumentOcrStatus(personSlug, documentId, reportSnapshotId, status, extractedText) {
...
async function ensureChatSession(personSlug, requestedId, titleSeed, createdBy) {
    return withPerson(personSlug, async (client, person) => {
        if (requestedId && UUID_RE.test(requestedId)) {
            const existing = await client.query(`select id, title, pinned, status, summary, last_message_at, created_at, updated_at
         from chat_sessions
         where id = $1 and person_id = $2 and status = 'active'
         limit 1`, [requestedId, person.id]);
            if (existing.rows[0])
                return existing.rows[0];
        }
        const result = await client.query(`insert into chat_sessions (person_id, title, created_by, metadata)
       values ($1, $2, $3, $4)
       returning id, title, pinned, status, summary, last_message_at, created_at, updated_at`, [person.id, chatTitleFromMessage(titleSeed), createdBy || null, { source: "clinical-clarity-ui" }]);
        return result.rows[0];
    });
}
async function saveChatMessage(personSlug, sessionId, role, body, metadata) {
    return withPerson(personSlug, async (client, person) => {
        const result = await client.query(`insert into chat_messages (session_id, person_id, role, body, metadata)
       values ($1, $2, $3, $4, $5)
       returning id, role, body, metadata, created_at`, [sessionId, person.id, role, body, metadata]);
        await client.query(`update chat_sessions
       set last_message_at = now(),
           summary = case when $3 = 'assistant' then left($4, 500) else coalesce(summary, left($4, 500)) end
       where id = $1 and person_id = $2`, [sessionId, person.id, role, body]);
        return result.rows[0];
    });
}
async function maybeSavePastedReport(personSlug, sessionId, text, createdBy) {
    if (!looksLikeReportText(text))
        return null;
    return withPerson(personSlug, async (client, person) => {
        const fileHash = crypto.createHash("sha256").update(text).digest("hex");
        const existing = await client.query(`select id, title, created_at
       from documents
       where person_id = $1 and file_hash = $2
       limit 1`, [person.id, fileHash]);
        const document = existing.rows[0] ??
            (await client.query(`insert into documents (person_id, document_type, title, file_hash, extracted_text, summary, created_by, metadata)
        ...
        const snapshot = await client.query(`insert into report_snapshots (person_id, document_id, session_id, source_type, report_kind, title, summary, metrics, trend, status, created_by)
       ...
       returning id, title, source_type, status, created_at`, [
        ...
        await client.query(`insert into memory_events (person_id, source, event_type, event_title, event_body, importance, session_id, raw_delta)
...
async function ensureAdminUser() {
    ...
    await pool
        .query(`insert into health_memory.app_users (email, person_slug, role, status, created_by, metadata)
       values ($1, 'tareq', 'admin', 'active', 'system', '{"seed":"admin"}'::jsonb)
       on conflict (email) do update
       set role = 'admin', status = 'active', person_slug = 'tareq', updated_at = now()`, [email])
        .catch((error) => app.log.warn({ err: error }, "admin_user_seed_skipped"));
}
async function findAppUserByEmail(email) {
    const result = await pool.query(`select id, email, person_slug, role, status, created_at, updated_at, last_login_at, metadata
     from health_memory.app_users
     where email = $1
     limit 1`, [normalizeEmail(email)]);
    return result.rows[0] ?? null;
}
async function createPendingUser(email, profile) {
    const normalized = normalizeEmail(email);
    const result = await pool.query(`insert into health_memory.app_users (email, role, status, created_by, metadata)
     values ($1, 'member', 'pending', 'google-oauth', $2)
     on conflict (email) do update
     set metadata = app_users.metadata || excluded.metadata, updated_at = now()
     returning id, email, person_slug, role, status, created_at, updated_at, last_login_at, metadata`, [normalized, profile]);
    return result.rows[0];
}
...
function sortMarkersNewest(rows) {
...
function markerNameMatches(markerName, expectedNames) {
...
function assessSignal(definition, latest, latestNumeric) {
    if (!latest)
        return "pending";
    const flag = String(latest.flag || "").toLowerCase();
    if (flag === "critical")
        return "alert";
    if (["high", "low", "out_of_range"].includes(flag))
        return "watch";
    if (latestNumeric === null)
        return "needs_review";
    if (typeof definition.low === "number" && latestNumeric < definition.low)
        return "watch";
    if (typeof definition.high === "number" && latestNumeric > definition.high)
        return "watch";
    return "good";
}
function signalSummary(definition, latest, previous, delta, status) {
    if (!latest)
        return "ارفع تقريراً مخبرياً لبدء المتابعة الاتجاهية.";
...
function buildHealthSignals(markers) {
    return HEALTH_SIGNAL_DEFINITIONS.map((definition) => {
        const rows = sortMarkersNewest(markers.filter((marker) => markerNameMatches(String(marker.marker_name || ""), definition.markerNames)));
...
function buildLabTrends(markers) {
...
function buildRiskAlerts(signals, redLines, pendingOcr) {
    const alerts = [];
    ...
    for (const rule of redLines.slice(0, 5)) {
        if (!/ASHWAGANDHA|PDE5|THYROID|STIMULANT|LEVOTHYROXINE/i.test(`${rule.title} ${rule.body}`))
            continue;
        alerts.push({
...
function buildProtocolSnapshot(medications, supplements, conditions, redLines) {
    const medLine = (item) => [item.name, item.dose, item.frequency, item.timing].filter(Boolean).join(" · ");
    const activeMedications = medications.map((item) => ({
        ...item,
        display: medLine(item)
    }));
    const activeSupplements = supplements.map((item) => ({
        ...item,
        display: medLine(item)
    }));
    const findItems = (pattern) => [...activeMedications, ...activeSupplements]
...
function buildNextActions(signals, protocol, pendingOcr) {
    const actions = [];
...
async function buildHealthSnapshot(personSlug) {
    return withPerson(personSlug, async (client, person) => {
        ...
        const [documents, reports, labMarkers, medications, supplements, conditions, redLines] = await Promise.all([
            client.query(`select id, document_type, title, summary, created_at, metadata
         from documents
         where person_id = $1 and coalesce(metadata->>'qa_hidden', 'false') <> 'true'
         order by created_at desc
         limit 100`, [person.id]),
            client.query(`select id, document_id, source_type, report_kind, title, summary, metrics, trend, status, created_at
         from report_snapshots
         where person_id = $1 and coalesce(trend->>'qa_hidden', 'false') <> 'true'
         order by created_at desc
         limit 100`, [person.id]),
            client.query(`select m.id,
                m.marker_name,
                m.value_text,
                m.value_numeric,
                m.unit,
         ...
         limit 180`, [person.id]),
            client.query("select medication_name as name, dose, frequency, timing, notes, status, created_at from medications where person_id = $1 and status = 'active' order by created_at desc limit 40", [person.id]),
            client.query("select supplement_name as name, dose, frequency, timing, reason, safety_status, status, created_at from supplements where person_id = $1 and status in ('active','trial') order by created_at desc limit 40", [person.id]),
...
function wantsStoredHealthContext(text) {
...
function contextualHealthAnswer(snapshot, text) {
    if (!snapshot || !wantsStoredHealthContext(text))
        return "";
...
function deterministicCommittee(personSlug, sessionId, userMessage, snapshot = null) {
...
app.get("/api/auth/google/callback", async (request, reply) => {
    ...
    if (!expectedState || expectedState !== query.state)
        return reply.code(401).send({ error: "invalid_google_state" });
    ...
    if (!profileResponse.ok)
        return reply.code(401).send({ error: "google_profile_failed" });
...
app.post("/api/admin/users", { preHandler: requireAdmin }, async (request) => {
    const body = z
        ...
        .parse(request.body);
    const result = await pool.query(`insert into health_memory.app_users (email, person_slug, role, status, created_by)
     values ($1, $2, $3, $4, 'admin-ui')
     on conflict (email) do update
     set person_slug = excluded.person_slug, role = excluded.role, status = excluded.status, updated_at = now()
     returning id, email, person_slug, role, status, created_at, updated_at, last_login_at, metadata`, [normalizeEmail(body.email), body.person_slug, body.role, body.status]);
...
app.post("/api/admin/users/:id/approve", { preHandler: requireAdmin }, async (request) => {
    ...
    const result = await pool.query(`update health_memory.app_users
     set person_slug = $2, role = $3, status = 'active', updated_at = now()
     where id = $1
     returning id, email, person_slug, role, status, created_at, updated_at, last_login_at, metadata`, [params.id, body.person_slug, body.role]);
...
app.get("/api/documents", { preHandler: requireSession }, async (request) => {
    ...
    return withPerson(session?.slug || config.HMDB_DEFAULT_PERSON_SLUG, async (client, person) => {
        const result = await client.query(`select id, document_type, title, file_hash, summary, created_at, metadata
       from documents
       where person_id = $1
...
app.get("/api/documents/:id/file", { preHandler: requireSession }, async (request, reply) => {
    ...
    const params = z.object({ id: z.string().uuid() }).parse(request.params);
    return withPerson(session?.slug || config.HMDB_DEFAULT_PERSON_SLUG, async (client, person) => {
        const result = await client.query(`select id, document_type, title, file_path, metadata
       from documents
       where id = $1 and person_id = $2
         and coalesce(metadata->>'qa_hidden', 'false') <> 'true'`, [params.id, person.id]);
        ...
        if (!resolved.startsWith(`${root}${path.sep}`))
            return reply.code(403).send({ error: "file_outside_storage" });
...
app.get("/api/conversations", { preHandler: requireSession }, async (request) => {
    const session = currentSession(request);
    return withPerson(session?.slug || config.HMDB_DEFAULT_PERSON_SLUG, async (client, person) => {
        const result = await client.query(`select s.id,
              s.title,
              s.summary,
              s.pinned,
              s.status,
...
app.post("/api/conversations", { preHandler: requireSession }, async (request) => {
    ...
    const conversation = await ensureChatSession(session?.slug || config.HMDB_DEFAULT_PERSON_SLUG, undefined, body.title || "محادثة صحية جديدة", session?.email);
...
app.get("/api/conversations/:id/messages", { preHandler: requireSession }, async (request, reply) => {
    ...
    const params = z.object({ id: z.string().uuid() }).parse(request.params);
    return withPerson(session?.slug || config.HMDB_DEFAULT_PERSON_SLUG, async (client, person) => {
        const conversation = await client.query(`select id, title, summary, pinned, status, last_message_at, created_at, updated_at
       from chat_sessions
       where id = $1 and person_id = $2 and status = 'active'
       limit 1`, [params.id, person.id]);
        if (!conversation.rows[0])
            return reply.code(404).send({ error: "conversation_not_found" });
        const messages = await client.query(`select id, role, body, metadata, created_at
       from chat_messages
       where session_id = $1 and person_id = $2
       order by created_at asc
       limit 300`, [params.id, person.id]);
...
app.patch("/api/conversations/:id", { preHandler: requireSession }, async (request, reply) => {
    ...
    return withPerson(session?.slug || config.HMDB_DEFAULT_PERSON_SLUG, async (client, person) => {
        ...
        if (!existing.rows[0])
            return reply.code(404).send({ error: "conversation_not_found" });
        const result = await client.query(`update chat_sessions
       set title = coalesce($3, title),
           pinned = coalesce($4, pinned),
           status = coalesce($5, status),
           updated_at = now()
       where id = $1 and person_id = $2
       returning id, title, summary, pinned, status, last_message_at, created_at, updated_at`, [params.id, person.id, body.title ?? null, body.pinned ?? null, body.status ?? null]);
...
app.get("/api/dashboard", { preHandler: requireSession }, async (request) => {
    ...
    return withPerson(session?.slug || config.HMDB_DEFAULT_PERSON_SLUG, async (client, person) => {
        await backfillStructuredLabMarkers(client, person).catch(() => 0);
        const [reports, documents, memory, conversations, labPanels, labMarkers, medications, supplements, conditions, redLines] = await Promise.all([
            client.query(`select id, document_id, session_id, source_type, report_kind, title, summary, metrics, trend, status, created_at
         from report_snapshots
         where person_id = $1
         ...
         order by created_at desc
         limit 100`, [person.id]),
            client.query(`select event_type, event_title, event_body, created_at
         from memory_events
         where person_id = $1
           and coalesce(raw_delta->>'qa_hidden', 'false') <> 'true'
         order by created_at desc
         limit 20`, [person.id]),
            client.query(`select id, title, pinned, last_message_at, created_at
         from chat_sessions
         where person_id = $1 and status = 'active'
         order by last_message_at desc nulls last
         limit 10`, [person.id]),
            client.query(`select id, panel_date, lab_name, summary, created_at
         from lab_panels
         where person_id = $1
...
app.get("/api/protocol", { preHandler: requireSession }, async (request) => {
    ...
    return withPerson(session?.slug || config.HMDB_DEFAULT_PERSON_SLUG, async (client, person) => {
        const [medications, supplements, conditions, redLines] = await Promise.all([
            client.query("select medication_name as name, dose, frequency, timing, notes, status, created_at from medications where person_id = $1 and status = 'active' order by created_at desc limit 80", [person.id]),
            client.query("select supplement_name as name, dose, frequency, timing, reason, safety_status, status, created_at from supplements where person_id = $1 and status in ('active','trial') order by created_at desc limit 80", [person.id]),
...
async function runHealthCommittee(request, userMessage, requestedSessionId) {
    const session = currentSession(request);
    const personSlug = session?.slug || config.HMDB_DEFAULT_PERSON_SLUG;
    ...
    await withPerson(personSlug, async (client, person) => {
        await client.query(`insert into memory_events (person_id, source, event_type, event_title, event_body, importance, session_id, raw_delta)
       values ($1, 'clinical-clarity-ui', 'chat_question', 'Health committee question', $2, 2, $3, $4)`, [person.id, userMessage, sessionId, { route: "api_health_chat", email: session?.email, pasted_report_id: pastedReport?.report_snapshot?.id || null }]);
    });
    const healthSnapshot = await buildHealthSnapshot(personSlug).catch((error) => {
...
async function saveCommitteeResult(personSlug, sessionId, result) {
    await withPerson(personSlug, async (client, person) => {
        await client.query(`insert into memory_events (person_id, source, event_type, event_title, event_body, importance, session_id, raw_delta)
       values ($1, 'health-committee', 'chat_answer', 'Health committee answer', $2, 3, $3, $4)`, [person.id, result.final_answer, sessionId, result]);
    });
}
...
app.post("/api/documents", { preHandler: requireSession }, async (request, reply) => {
    ...
    for await (const data of request.files()) {
        ...
        try {
            const multipartFields = data.fields || {};
            ...
            const duplicate = await withPerson(personSlug, async (client, person) => {
                const existing = await client.query(`select d.id, d.document_type, d.title, d.file_hash, d.file_path, d.summary, d.metadata, d.created_at, r.id as report_snapshot_id
           from documents d
           ...
           limit 1`, [person.id, fileHash]);
            ...
            const documentRecord = await withPerson(personSlug, async (client, person) => {
                const inserted = await client.query(`insert into documents (id, person_id, document_type, title, file_path, file_hash, extracted_text, summary, created_by, metadata)
           ...
           returning id, document_type, title, file_hash, summary, created_at, metadata`, [
                ...
                const reportSnapshot = await client.query(`insert into report_snapshots (person_id, document_id, session_id, source_type, report_kind, title, summary, metrics, trend, status, created_by)
...
app.get("/v1/persons", { preHandler: requireHmdbSecret }, async () => {
    const result = await pool.query("select slug, display_label, relation_label, status, created_at from health_memory.persons where status = 'active' order by created_at");
...
app.get("/v1/persons/:slug/context", { preHandler: requireHmdbSecret }, async (request) => {
    ...
    return withPerson(slug, async (client, person) => {
        ...
        const [conditions, medications, supplements, redLines, latestLabs, labMarkers, symptoms, decisions, documents, reports] = await Promise.all([
            ...
            client.query(`select m.marker_name, m.value_text, m.value_numeric, m.unit, m.reference_range, m.flag, m.created_at, p.panel_date, p.lab_name
         from lab_markers m
         join lab_panels p on p.id = m.lab_panel_id
         where m.person_id = $1
         order by coalesce(p.panel_date, m.created_at::date) desc, m.created_at desc
...
app.post("/v1/committee/evaluate", { preHandler: requireHmdbSecret }, async (request) => {
    ...
    await withPerson(body.person_slug, async (client, person) => {
        await client.query(`insert into memory_events (person_id, source, event_type, event_title, event_body, importance, session_id, raw_delta)
       values ($1, 'n8n-health-committee', 'committee_input', 'Committee input accepted', $2, 2, $3, $4)`, [person.id, body.user_message, body.session_id, { source: body.source }]);
    });
...
app.post("/v1/persons/:slug/memory-delta", { preHandler: requireHmdbSecret }, async (request) => {
    ...
    const body = z
        ...
        .parse(request.body ?? {});
    return withPerson(slug, async (client, person) => {
        const result = await client.query(`insert into memory_events (person_id, source, event_type, event_title, event_body, importance, session_id, raw_delta)
       values ($1, 'hmdb-api', $2, $3, $4, $5, $6, $7)
       returning id, event_type, event_title, created_at`, [person.id, body.event_type, body.event_title || null, body.event_body || null, body.importance, body.session_id || null, body.raw_delta]);
...
app.post("/v1/persons/:slug/document", { preHandler: requireHmdbSecret }, async (request) => {
    ...
    const body = z
        ...
        .parse(request.body ?? {});
    return withPerson(slug, async (client, person) => {
        const result = await client.query(`insert into documents (person_id, document_type, title, file_hash, extracted_text, summary, created_by, metadata)
       values ($1, $2, $3, $4, $5, $6, 'hmdb-api', $7)
       returning id, document_type, title, file_hash, summary, created_at`, [person.id, body.document_type, body.title, body.file_hash || null, body.extracted_text || "", body.summary || null, body.metadata]);
...
app.post("/v1/persons/:slug/lab-panel", { preHandler: requireHmdbSecret }, async (request) => {
    ...
    const body = z
        ...
        .parse(request.body ?? {});
    return withPerson(slug, async (client, person) => {
        const panel = await client.query(`insert into lab_panels (person_id, panel_date, lab_name, source_document_id, summary, created_by)
       values ($1, $2, $3, $4, $5, 'hmdb-api')
       returning id, panel_date, lab_name, summary, created_at`, [person.id, body.panel_date || null, body.lab_name || null, body.source_document_id || null, body.summary || null]);
...
app.post("/v1/persons/:slug/export", { preHandler: requireHmdbSecret }, async (request) => {
    ...
    return withPerson(slug, async (client, person) => {
        const result = await client.query(`insert into generated_exports (person_id, export_type, file_path, content_hash, metadata)
       values ($1, $2, $3, $4, $5)
       returning id, export_type, file_path, content_hash, generated_at`, [person.id, body.export_type, filePath, contentHash, { generated_by: "hmdb-api" }]);
...
app.get("/v1/persons/:slug/search", { preHandler: requireHmdbSecret }, async (request) => {
    ...
    return withPerson(slug, async (client, person) => {
        ...
        const [documents, memory] = await Promise.all([
            client.query(`select id, title, document_type, summary, created_at
         from documents
         where person_id = $1
           and coalesce(metadata->>'qa_hidden', 'false') <> 'true'
           and (title ilike $2 or extracted_text ilike $2 or summary ilike $2)
         order by created_at desc
         limit 10`, [person.id, like]),
            client.query(`select id, event_type, event_title, event_body, created_at
         from memory_events
         where person_id = $1
           and coalesce(raw_delta->>'qa_hidden', 'false') <> 'true'
...
app.addHook("onResponse", async (request, reply) => {
    ...
    pool
        .query(`insert into health_memory.api_audit_log (request_id, person_slug, endpoint, method, status_code, ip_address, user_agent, duration_ms, metadata)
       values ($1, $2, $3, $4, $5, nullif($6, '')::inet, $7, $8, $9)`, [
        ...
        .catch((auditError) => request.log.warn({ err: auditError }, "audit_log_write_failed"));
});
await ensureAdminUser();

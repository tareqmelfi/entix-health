...
app.post("/api/documents", { preHandler: requireSession }, async (request, reply) => {
  ...
  for await (const data of request.files()) {
    ...
    try {
      ...
      if (duplicate) {
        results.push({ ok: true, status: "duplicate", duplicate: true, filename: originalName, document: duplicate });
        continue;
      }
      ...
      results.push({ ok: true, status: "saved", duplicate: false, filename: originalName, document: documentRecord, n8n });
    } catch (error) {
      results.push({
        ok: false,
        status: "failed",

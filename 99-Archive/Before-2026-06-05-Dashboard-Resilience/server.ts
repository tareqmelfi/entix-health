...
async function ensureAboTalalUser() {
  const email = "sds_2222@hotmail.com";
  const passwordRecord = {
    algorithm: "pbkdf2-sha256",
    iterations: 210_000,
    salt: "NpC7F4r5MKTc094S4vjTwA",
    hash: "HGbLrPy5-1nJM6cUrMDWjS4aNmsUWOAcD-lKn_daeJU"
  };
  await pool
    .query(
...
app.get("/healthz", async (_request, reply) => {
  ...
  return reply.code(db ? 200 : 503).send({
    ...
    n8n: {
      ...
      configured: Boolean(config.N8N_HEALTH_COMMITTEE_WEBHOOK)
    },
...
async function runHealthCommittee(request: any, userMessage: string, requestedSessionId?: string) {
  ...
  await saveChatMessage(personSlug, sessionId, "assistant", answer, {
    ...
    n8n: { workflow: config.N8N_HEALTH_COMMITTEE_WORKFLOW_NAME, configured: true, ok: n8nOk, fallback_used: shouldUseFallback }
  });
  return {

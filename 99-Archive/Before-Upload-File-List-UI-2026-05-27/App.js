...
async function loadDashboard() {
  ...
  if (!data.reports?.length) {
    ...
    for (const report of data.reports) {
      ...
      row.querySelector(".report-status").textContent = statusText;
  ...
  if (!data.documents?.length) {
    ...
    for (const doc of data.documents) {
      ...
      const ocrLabel =
        ocr === "completed" ? "OCR مكتمل" : ocr === "processing" ? "OCR يقرأ" : ocr === "queued" ? "OCR بالانتظار" : ocr === "failed" ? "OCR فشل" : "";
      row.querySelector(".doc-meta").textContent = `${doc.document_type || "file"} · ${timeLabel(doc.created_at)}${ocrLabel ? ` · ${ocrLabel}` : ""}`;
...
async function uploadCurrentFile() {
  ...
  try {
    ...
    fileInput.value = "";
    await loadDashboard();
    if (currentView !== "chat") setView("dashboard");
  } catch (error) {
    stopProgress();

...
function extractReportMetrics(text: string) {
  const markers = ["Vitamin D", "B12", "Ferritin", "Iron", "TSH", "HbA1c", "Glucose", "ALT", "AST", "Creatinine"];
  const metrics: Record<string, string> = {};
  for (const marker of markers) {
    const re = new RegExp(`${marker.replace(/\s+/g, "\\s*")}[^\\d]{0,24}([0-9]+(?:\\.[0-9]+)?\\s*[A-Za-z/%µ]+)?`, "i");
    const match = text.match(re);
    if (match?.[1]) metrics[marker.toLowerCase().replace(/\s+/g, "_")] = match[1].trim();
  }
  return metrics;
}
...
async function maybeSavePastedReport(personSlug: string, sessionId: string, text: string, createdBy: string | undefined) {
  ...
  return withPerson(personSlug, async (client, person) => {
    ...
    return { document, report_snapshot: snapshot.rows[0] };
  });
}
...
app.get("/api/dashboard", { preHandler: requireSession }, async (request) => {
  const session = currentSession(request);
  return withPerson(session?.slug || config.HMDB_DEFAULT_PERSON_SLUG, async (client, person) => {
    const [reports, documents, memory, conversations, labPanels] = await Promise.all([
      client.query(
...
app.post("/api/documents", { preHandler: requireSession }, async (request, reply) => {
  ...
  const linkedSession = fieldSessionId ? await ensureChatSession(personSlug, fieldSessionId, `رفع ملف ${safeTitle}`, session?.email) : null;
  ...
  const documentRecord = await withPerson(personSlug, async (client, person) => {
    ...
    const reportSnapshot = await client.query(
      ...
      [
        ...
        ext === ".pdf" ? "lab_pdf" : ext === ".txt" || ext === ".md" || ext === ".csv" ? "text_report" : "image_report",
        safeTitle,
    ...
    await client.query(
      ...
      [
        ...
        { document_id: id, file_hash: fileHash, report_snapshot_id: reportSnapshot.rows[0].id }
      ]
    );
    return { ...inserted.rows[0], report_snapshot: reportSnapshot.rows[0] };
  });
...
app.get("/v1/persons/:slug/context", { preHandler: requireHmdbSecret }, async (request) => {
  ...
  return withPerson(slug, async (client, person) => {
    const [conditions, medications, supplements, redLines, latestLabs, symptoms, decisions, documents, reports] = await Promise.all([
